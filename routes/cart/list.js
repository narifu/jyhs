        ...
        request.app.db.query(countSql, (err, count_res) => {
            if(err) {
                ...
                if(count_res[0].count>0){
                    ...
                    const select = `select c.*,g.id group_id,g.name group_name,g.status group_status from cart c,group_bill g where c.group_bill_id=g.id and c.status=1 and  c.user_id=${request.query.user_id} order by id limit ${from},${request.query.size}`;
