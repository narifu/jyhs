...
module.exports = {
    ...
    handler(request, reply) {
        const update = `update group_bill set province='${request.payload.province}',city='${request.payload.city}', flash_desc='${request.payload.flash_desc}', isflash='${request.payload.is_flash}',  description='${request.payload.description}', freight=${request.payload.freight}, pay_description='${request.payload.pay_description}',pay_name='${request.payload.pay_name}',pay_count='${request.payload.pay_count}',pay_type='${request.payload.pay_type}', contacts='${request.payload.contacts}',phone='${request.payload.phone}',end_date=${request.payload.end_date},pickup_date=${request.payload.pickup_date}, pickup_address='${request.payload.pickup_address}' where id=${request.payload.id}`;
    ...
    config: {
        ...
        pre: [
            ...
            {
                method(request, reply) {
                    if(moment(request.payload.pickup_date+"","YYYYMMDDhmmss").isAfter(moment())){
                        reply(true);
                    } else {
                        reply(Boom.notAcceptable('提货日期小于今天'));
                    }
                }
            }
        ]
